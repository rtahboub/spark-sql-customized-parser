 ...
 */
abstract class MyAbstractSqlParser extends ParserInterface with Logging {
 ...
 */
class MyCatalystSqlParser(conf: SQLConf) extends MyAbstractSqlParser {
  val astBuilder = new MyAstBuilder(conf)
}

/** For test-only. */
object MyCatalystSqlParser extends MyAbstractSqlParser {
  val astBuilder = new MyAstBuilder(new SQLConf())
  logInfo("Hello from MyAbstractSqlParser")
}
